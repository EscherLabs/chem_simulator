device_components.push(
    {
        ...
        //       if(parseInt(i[keys[0]]) >= gform.instances['FTIS'].get('sample_holder')['range_start'] && parseInt(i[keys[0]]) <= gform.instances['FTIS'].get('sample_holder')['range_end']){
        //         x.push(i[keys[0]]);
        //         y.push(i[keys[1]]);
        //       }
        //     })
          ...
          },
          {
            ...
            "handler":function(e){
              ...
              if(!e.form.validate() || __.validate(__.findComponent().validationFields))return false;
                ...
                // if(parseInt(i[keys[0]]) >= gform.instances['FTIS'].get('sample_holder')['range_start'] && parseInt(i[keys[0]]) <= gform.instances['FTIS'].get('sample_holder')['range_end']){
                let url = 'assets/data/ftis/'+e.form.get('file')+'.csv';
                if(e.form.get('file') == "HCl")url = 'assets/data/ftis/'+e.form.get('file')+'.csv';
                resources.data = [{
                  label: e.form.get('file'),
                  url: url,
                  skip:1,
                  keys:['Wavenumber (cm-1)','% Transmittance'],
                  min:gform.instances['FTIS'].get('sample_holder')['range_start'],
                  max:gform.instances['FTIS'].get('sample_holder')['range_end'],
                }]
