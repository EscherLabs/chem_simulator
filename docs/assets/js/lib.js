        ...
        let settings = _.defaultsDeep({},activeComponent.chartConfig,{
          ...
          axis: {
              x: {
                tick: {
                  format(d) {
                    return (__.findComponent().reverse)?this.data.xs[_.keys(this.data.xs)[0]][this.data.xs[_.keys(this.data.xs)[0]].length-(1+this.data.xs[_.keys(this.data.xs)[0]].indexOf(d))]:d;
                  }
                },
                  label: keys[0]
              },
